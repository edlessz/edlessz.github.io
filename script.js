...
const lastfm = async (user, method) => {
    const response = await fetch(`https://ws.audioscrobbler.com/2.0/?method=${method}&user=${user}&api_key=1f62f0c0081f372b2d4979b2ae154cbd&format=json`);
...
lastfm('edscamera', 'user.getRecentTracks').then(data => {
    ...
    $("#track-image").src = newestTrack.image.find(x => x.size === "large")['#text'];
